...
def options(opt):
    ...
    opt.load("flags eigen mfem ipopt nlopt python", tooldir="waf_tools")
...
def configure(cfg):
    ...
    cfg.load("flags eigen mfem ipopt nlopt python", tooldir="waf_tools")
...
def build(bld):
    # Library name
    bld.get_env()["libname"] = "GpManifold"
